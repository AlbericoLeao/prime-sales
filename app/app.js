...
function renderCatalogo() {
  ...
  return head('Catálogo', 'Monte pedidos rapidamente pelo celular.') +
    `<section class="card"><label>Cliente<select id="pedido-cliente"><option value="">Selecione cliente da carteira</option>${clientes.map(c=>`<option value="${c.id}" ${state.pedidoClienteId===c.id?'selected':''}>${escapeHtml(c.nome)}</option>`).join('')}</select></label><label>Observações Comerciais<textarea id="pedido-obs" placeholder="Prazo, frete, condições especiais, entrega parcial...">${escapeHtml(state.pedidoObs)}</textarea></label><div class="actions"><button class="btn" data-save-draft>Salvar rascunho</button></div></section>` + productFiltersHtml() + productListHtml(activeProducts(), false) +
...
function clienteForm(c = {}) {
  return `<section class="card" id="cliente-form-card"><div class="card-title"><h3>${c.id ? 'Editar cliente' : 'Novo cliente'}</h3></div><input type="hidden" id="cli-id" value="${escapeHtml(c.id || '')}"><label>Nome / Razão social<input id="cli-nome" value="${escapeHtml(c.nome || c.razaoSocial || '')}"></label><div class="form-row"><label>CNPJ / CPF<input id="cli-doc" value="${escapeHtml(c.doc || c.cnpj || '')}"></label><label>Telefone<input id="cli-tel" value="${escapeHtml(c.tel || c.telefone || '')}"></label></div><div class="form-row"><label>Cidade<input id="cli-cidade" value="${escapeHtml(c.cidade || '')}"></label><label>Estado<input id="cli-estado" maxlength="2" value="${escapeHtml(c.estado || '')}"></label></div><label>Vendedor responsável<select id="cli-vendedor"><option value="" ${!c.vendedorId?'selected':''}>Carteira geral</option>${state.vendedores.map(v=>`<option value="${v.id}" ${c.vendedorId===v.id?'selected':''}>${escapeHtml(v.nome||v.email)}</option>`).join('')}</select></label><label>Observações<textarea id="cli-obs">${escapeHtml(c.obs || '')}</textarea></label><div class="actions"><button class="btn primary" data-save-client>Salvar cliente</button><button class="btn" data-cancel-client-form>Cancelar</button></div></section>`;
}
...
async function saveDraft() {
  return saveOrder('rascunho');
}

async function sendOrder() {
  return saveOrder('enviado');
}
...
function bindPageEvents() {
  ...
  $$('[data-pdf]').forEach(b => b.onclick = () => { const pedido = state.pedidos.find(p=>p.id===b.dataset.pdf); if (pedido) gerarPedidoPDF(pedido); });
  ...
  $('[data-new-client]')?.addEventListener('click', () => { state.clienteEditId = ''; state.clienteFormOpen = true; renderPage(); });
  $$('[data-edit-client]').forEach(b => b.onclick = () => { state.clienteEditId = b.dataset.editClient; state.clienteFormOpen = true; renderPage(); });
  $$('[data-transfer-client]').forEach(b => b.onclick = () => { state.clienteEditId = b.dataset.transferClient; state.clienteFormOpen = true; renderPage(); });
  $('[data-save-client]')?.addEventListener('click', saveClienteForm);
  $('[data-cancel-client-form]')?.addEventListener('click', () => { state.clienteFormOpen = false; state.clienteEditId = ''; renderPage(); });
...
function updateDiscount(id, pct) {
  ...
  if (!item || !item.qty) return renderPage();
...
async function saveClienteForm() {
  ...
  const vendedorId = $('#cli-vendedor')?.value || '';
  const vendedorNomeResp = vendedorId ? vendedorNome(vendedorId) : '';
  const data = {
    nome,
    razaoSocial: nome,
    doc: $('#cli-doc')?.value.trim() || '',
    cnpj: $('#cli-doc')?.value.trim() || '',
    tel: $('#cli-tel')?.value.trim() || '',
    telefone: $('#cli-tel')?.value.trim() || '',
    cidade: $('#cli-cidade')?.value.trim() || '',
    estado: ($('#cli-estado')?.value.trim() || '').toUpperCase(),
    obs: $('#cli-obs')?.value.trim() || '',
    vendedorId,
    vendedorNome: vendedorNomeResp,
    status: 'ativo',
    atualizadoEm: fb.serverTimestamp()
  };
  if (id) await fb.updateDoc(ref('clientes', id), data);
  else await fb.addDoc(col('clientes'), { ...data, criadoEm: fb.serverTimestamp() });
  if (vendedorId) await notifyUser(vendedorId, 'Cliente atribuído', `${nome} está na sua carteira.`);
